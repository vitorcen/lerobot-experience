#!/usr/bin/env python3
"""Quick Pi0 prompt test — run a single LIBERO episode and save video."""
import argparse
import subprocess
import sys
import time
...
def main():
    p = argparse.ArgumentParser(description="Pi0 interactive prompt test")
    ...
    p.add_argument("--policy", default="lerobot/pi0_libero_finetuned", help="HF model ID or local path")
    p.add_argument("--device", default="cuda", help="cuda or cpu")
    args = p.parse_args()

    ts = time.strftime("%Y%m%d_%H%M%S")
    out = Path(f"outputs/eval/pi0_interactive_{ts}")
